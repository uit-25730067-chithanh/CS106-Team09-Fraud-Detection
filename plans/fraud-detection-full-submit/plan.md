...
| 07 | [Report & PPT](./phase-07-report-ppt.md) | Duy + Hôn | Sprint 2–4* | `pending` | Word/PDF Chương 1–6 ✅ 28 trang (Duy) + PPT 15 slides ✅ (Hôn) + Slide học thuật ✅ (PR #13); Chương 7 và Abstract chưa viết |
...
>
> **Phase 07 — Duy & Hôn — checkpoint 04/09/2026:** Duy đã hoàn thành bản nháp Sprint 3 Chương 1–5, gồm Methodology và Results draft tái tính từ prediction artifacts; source + DOCX/PDF 24 trang giữ format Sprint 2, mục lục có đủ các mục nhỏ 4.1–4.9 và 5.1–5.6, Chương 5 có Hình 5.1 và Hình 5.2, không còn dấu chấm phẩy và đã qua visual QA toàn bộ trang. Bản Word nay được sinh tự động từ `report-source.md` bằng `build_report.py`, không chỉnh tay. Chương 5 tái lập được bằng `run_report_metrics.py` → `reports/ch5_metrics_recomputed.csv`, có cột Average Precision và Mục 5.5 về mức đóng góp đặc trưng. Bộ Slide PowerPoint chính thức 15 slides của Hôn đã hoàn tất tại `reports/`; bộ slide học thuật của Thanh nằm tại `slide/` để tham khảo và backup Q&A. Chương 6 Thảo luận đã hoàn thành, gồm phần quy chiếu Precision về tỷ lệ gian lận gốc của PaySim và phân tích khoảng tin cậy cho thấy khác biệt giữa bốn biến thể học có giám sát chưa đủ tin cậy để xếp hạng. Phase vẫn `pending` chờ Phase 05 xuất comparison CSV cùng ROC/PR figures để đối chiếu, và chờ hoàn thiện Chương 7 cùng Abstract.
>
> **PM Audit — Hôn — 04/09/2026:** Scout toàn bộ file tree phát hiện 17 items cần xử lý: 3 notebooks thiếu (02, 03, 06), 2 scripts evaluation chưa tạo, `reports/model_comparison.csv` chưa tồn tại, RF `.pkl` bị gitignore, và Phase 08 checklist tham chiếu sai (`autoencoder.h5` → `autoencoder_meta.json`). Tiến độ tổng thể 5/9 phases = 55.6%, đúng Sprint timeline. *Cập nhật sau audit:* `reports/figures/` đã được tạo trên branch báo cáo của Duy và hiện có 2 hình dùng cho Chương 5.
